...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	if (sv_test)
	{
		...
		else if (text_in.substr(0, 11) == "!component " && player.isMod())
		{
			string[]@ tokens = text_in.split(" ");
			if (tokens.length == 3)
			{
				if (tokens[1].substr(0, 5).toUpper() == "DUMMY")
				{
					return true;
				}

				SpellComponent@ componentObj = GetComponentFromString(tokens[1], parseInt(tokens[2]));
				...
				CBlob@ component = server_CreateBlob("component", -1, pos);
				component.set("component object", @componentObj);

				CRules@ rules = getRules();

				CBitStream nameParams;
				nameParams.write_netid(component.getNetworkID());
        		nameParams.write_string("Grade " + componentObj.grade + " " + componentObj.ComponentID() + " rune");

				rules.SendCommand(rules.getCommandID("change comp name"), nameParams);

				CBitStream componentParams;
				componentParams.write_netid(component.getNetworkID());
				componentParams.write_string(tokens[1]);
				componentParams.write_s32(parseInt(tokens[2]));
